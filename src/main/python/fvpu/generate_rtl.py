...
def generate(module_name, working_dir, args):
    ...
    subprocess.run([mill, 'fvpu.run', '--', working_dir, module_name] + args, check=False, cwd=mill_dir)
    filenames = [os.path.join(working_dir, fn) for fn in os.listdir(working_dir) if fn[-3:] == '.sv']
    return filenames
