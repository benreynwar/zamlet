...
object ControlInstr {
  ...
  object Modes extends ChiselEnum {
    ...
    // Only really used in the resolvoing version we pass to famlet.
    val Incr = Value(4.U)
  ...
  def srcWidth(params: FamletParams): Int = {
    log2Ceil(scala.math.max(params.nARegs, params.nGRegs))
  }

  class BaseSrcType(params: FamletParams) extends Bundle {
    val mode = SrcMode()
    val value = UInt(srcWidth(params).W)
  }

  class ExtendedSrcType(params: FamletParams) extends Bundle {
    val resolved = Bool()
  ...
  class Expanded(params: FamletParams) extends Instr.Expanded(params) {
    ...
    def iterationsReadEnable(): Bool = {
      (
       (mode === Modes.LoopImmediate) ||
       (mode === Modes.LoopLocal) ||
       (mode === Modes.LoopGlobal)
      ) && !iterations.resolved
    }
    ...
    def rename(newIterations: UInt, newDst: UInt): Renamed = {
      val renamed = Wire(new Renamed(params))
      renamed.mode := mode
      renamed.iterations.resolved := iterations.resolved
      renamed.iterations.addr := newIterations
      renamed.iterations.value := iterations.value
      renamed.dst := newDst
  ...
  class RenamedSrcType(params: FamletParams) extends Bundle {
    val resolved = Bool()
  ...
  class Renamed(params: FamletParams) extends Instr.Expanded(params) {
    val mode = Modes()
    val iterations = new RenamedSrcType(params) // Where the number of iterations comes from.
    val dst = params.aPhysReg()                    // Where the loop index goes.
    val predicate = params.pReg()              // loop_index < iterations put here.
    val level = UInt(log2Ceil(params.nLoopLevels).W)
  }
