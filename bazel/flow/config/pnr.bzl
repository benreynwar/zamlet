...
PnRConfig = provider(
    ...
    fields = {
        ...
        "io_pin_order_cfg": "File - Pin order config for custom IO placement",
        ...
        "io_pin_v_extension": "Extend vertical IO pins outside die (um)",
        "io_pin_h_extension": "Extend horizontal IO pins outside die (um)",
        "io_pin_v_thickness_mult": "Vertical pin thickness multiplier",
        "io_pin_h_thickness_mult": "Horizontal pin thickness multiplier",
        "io_pin_placement_mode": "IO placement mode",
        "errors_on_unmatched_io": "Error on unmatched IO pins",

        # PDN
        "set_rc_verbose": "Echo set_rc commands",
        "pdn_skiptrim": "Skip metal trim step during pdngen",
        "pdn_core_ring": "Enable core ring around design",
        "pdn_enable_rails": "Enable rails in power grid",
        "pdn_horizontal_halo": "Horizontal halo around macros for PDN (um)",
        "pdn_vertical_halo": "Vertical halo around macros for PDN (um)",
        "pdn_multilayer": "Use multiple layers in power grid",
        ...
        "sta_extra_corner_tcl_file": "Extra OpenROAD corner Tcl file",
        "deduplicate_corners": "Deduplicate equivalent PnR corners",
        ...
        "pl_keep_resize_below_overflow": "Keep resize changes below this overflow",
        "pl_timing_driven": "Timing driven placement",
        ...
        "cts_balance_levels": "Balance CTS levels override",
        "cts_sink_buffer_max_cap_derate_pct": "CTS sink buffer max cap derate percentage",
        "cts_delay_buffer_derate_pct": "CTS delay buffer derate percentage",
        "cts_obstruction_aware": "CTS obstruction-aware buffering override",
        "cts_sink_clustering_enable": "Enable CTS sink clustering",
        "cts_macro_clustering_size": "Max macro sinks per cluster",
        "cts_macro_clustering_max_diameter": "Max macro cluster diameter in um",
        ...
        "cts_apply_ndr": "CTS non-default-rule strategy",
        ...
        "grt_antenna_repair_iters": "Max iterations for global antenna repairs",
        "grt_overflow_iters": "Max iterations for overflow convergence",
        "grt_antenna_repair_margin": "Margin % to over-fix antenna violations",
        "grt_antenna_repair_jumper_only": "Only use jumpers to fix antenna violations",
        "grt_antenna_repair_diode_only": "Only use antenna diodes to fix antenna violations",
        "drt_threads": "Threads for detailed routing",
        "drt_opt_iters": "Max optimization iterations",
        "drt_save_snapshots": "Save detailed routing iteration ODB snapshots",
        "drt_antenna_repair_iters": "Max post-DRT antenna repair iterations",
        "drt_antenna_repair_margin": "Margin % to over-fix post-DRT antenna violations",
        "drt_antenna_repair_jumper_only": "Only use jumpers for post-DRT antenna repair",
        "drt_antenna_repair_diode_only": "Only use diodes for post-DRT antenna repair",
        "drt_save_drc_report_iters": "Iteration interval for DRT DRC reports",
        "non_default_rules": "JSON object of non-default routing rules",
        "drt_assign_ndr": "JSON object mapping net regexes to NDR names",
        ...
        "pl_resizer_setup_gate_cloning": "Enable gate cloning for setup fixes",
        "pl_resizer_setup_buffering": "Enable setup rebuffering and load splitting",
        "pl_resizer_setup_buffer_removal": "Enable setup buffer removal",
        "pl_resizer_setup_repair_tns_pct": "Percentage of setup violating endpoints to repair",
        "pl_resizer_setup_max_util_pct": "Max setup repair utilization percentage",
        "pl_resizer_hold_repair_tns_pct": "Percentage of hold violating endpoints to repair",
        "pl_resizer_hold_max_util_pct": "Max hold repair utilization percentage",
        ...
        "grt_resizer_setup_gate_cloning": "Enable gate cloning for setup fixes",
        "grt_resizer_run_grt": "Run GRT after resizer steps",
        "grt_resizer_setup_buffering": "Enable setup rebuffering and load splitting",
        "grt_resizer_setup_buffer_removal": "Enable setup buffer removal",
        "grt_resizer_setup_repair_tns_pct": "Percentage of setup violating endpoints to repair",
        "grt_resizer_setup_max_util_pct": "Max setup repair utilization percentage",
        "grt_resizer_hold_repair_tns_pct": "Percentage of hold violating endpoints to repair",
        "grt_resizer_hold_max_util_pct": "Max hold repair utilization percentage",
...
PNR_ATTRS = {
    ...
    # PDN
    "set_rc_verbose": attr.bool(
        doc = "Echo set_rc commands in OpenROAD scripts",
        default = False,
    ),
    ...
    "sta_extra_corner_tcl_file": attr.label(
        doc = "Extra OpenROAD corner Tcl file",
        allow_single_file = [".tcl"],
    ),
    "deduplicate_corners": attr.bool(
        doc = "Deduplicate equivalent PnR corners",
        default = False,
    ),
    ...
    "cts_macro_clustering_max_diameter": attr.string(
        doc = "Max macro cluster diameter in um; empty leaves unset",
        default = "",
    ),
...
PNR_DEFAULTS = {
    ...
    "io_pin_v_extension": "0",
    "io_pin_h_extension": "0",
    ...
    "sta_extra_corner_tcl_file": None,
...
def _pnr_config_impl(ctx):
    return [PnRConfig(
        ...
        io_pin_order_cfg = ctx.file.io_pin_order_cfg,
        ...
        io_pin_v_extension = ctx.attr.io_pin_v_extension,
        io_pin_h_extension = ctx.attr.io_pin_h_extension,
        io_pin_v_thickness_mult = ctx.attr.io_pin_v_thickness_mult,
        io_pin_h_thickness_mult = ctx.attr.io_pin_h_thickness_mult,
        io_pin_placement_mode = ctx.attr.io_pin_placement_mode,
        errors_on_unmatched_io = ctx.attr.errors_on_unmatched_io,
        set_rc_verbose = ctx.attr.set_rc_verbose,
        pdn_skiptrim = ctx.attr.pdn_skiptrim,
        pdn_core_ring = ctx.attr.pdn_core_ring,
        pdn_enable_rails = ctx.attr.pdn_enable_rails,
        pdn_horizontal_halo = ctx.attr.pdn_horizontal_halo,
        pdn_vertical_halo = ctx.attr.pdn_vertical_halo,
        pdn_multilayer = ctx.attr.pdn_multilayer,
        pdn_cfg = ctx.file.pdn_cfg,
        ...
        sta_extra_corner_tcl_file = ctx.file.sta_extra_corner_tcl_file,
        deduplicate_corners = ctx.attr.deduplicate_corners,
        ...
        pl_keep_resize_below_overflow = ctx.attr.pl_keep_resize_below_overflow,
        pl_timing_driven = ctx.attr.pl_timing_driven,
        ...
        cts_balance_levels = ctx.attr.cts_balance_levels,
        cts_sink_buffer_max_cap_derate_pct = ctx.attr.cts_sink_buffer_max_cap_derate_pct,
        cts_delay_buffer_derate_pct = ctx.attr.cts_delay_buffer_derate_pct,
        cts_obstruction_aware = ctx.attr.cts_obstruction_aware,
        cts_sink_clustering_enable = ctx.attr.cts_sink_clustering_enable,
        cts_macro_clustering_size = ctx.attr.cts_macro_clustering_size,
        cts_macro_clustering_max_diameter = ctx.attr.cts_macro_clustering_max_diameter,
        ...
        cts_apply_ndr = ctx.attr.cts_apply_ndr,
        ...
        grt_antenna_repair_iters = ctx.attr.grt_antenna_repair_iters,
        grt_overflow_iters = ctx.attr.grt_overflow_iters,
        grt_antenna_repair_margin = ctx.attr.grt_antenna_repair_margin,
        grt_antenna_repair_jumper_only = ctx.attr.grt_antenna_repair_jumper_only,
        grt_antenna_repair_diode_only = ctx.attr.grt_antenna_repair_diode_only,
        drt_threads = ctx.attr.drt_threads,
        drt_opt_iters = ctx.attr.drt_opt_iters,
        drt_save_snapshots = ctx.attr.drt_save_snapshots,
        drt_antenna_repair_iters = ctx.attr.drt_antenna_repair_iters,
        drt_antenna_repair_margin = ctx.attr.drt_antenna_repair_margin,
        drt_antenna_repair_jumper_only = ctx.attr.drt_antenna_repair_jumper_only,
        drt_antenna_repair_diode_only = ctx.attr.drt_antenna_repair_diode_only,
        drt_save_drc_report_iters = ctx.attr.drt_save_drc_report_iters,
        non_default_rules = ctx.attr.non_default_rules,
        drt_assign_ndr = ctx.attr.drt_assign_ndr,
        ...
        pl_resizer_setup_gate_cloning = ctx.attr.pl_resizer_setup_gate_cloning,
        pl_resizer_setup_buffering = ctx.attr.pl_resizer_setup_buffering,
        pl_resizer_setup_buffer_removal = ctx.attr.pl_resizer_setup_buffer_removal,
        pl_resizer_setup_repair_tns_pct = ctx.attr.pl_resizer_setup_repair_tns_pct,
        pl_resizer_setup_max_util_pct = ctx.attr.pl_resizer_setup_max_util_pct,
        pl_resizer_hold_repair_tns_pct = ctx.attr.pl_resizer_hold_repair_tns_pct,
        pl_resizer_hold_max_util_pct = ctx.attr.pl_resizer_hold_max_util_pct,
        ...
        grt_resizer_setup_gate_cloning = ctx.attr.grt_resizer_setup_gate_cloning,
        grt_resizer_run_grt = ctx.attr.grt_resizer_run_grt,
        grt_resizer_setup_buffering = ctx.attr.grt_resizer_setup_buffering,
        grt_resizer_setup_buffer_removal = ctx.attr.grt_resizer_setup_buffer_removal,
        grt_resizer_setup_repair_tns_pct = ctx.attr.grt_resizer_setup_repair_tns_pct,
        grt_resizer_setup_max_util_pct = ctx.attr.grt_resizer_setup_max_util_pct,
        grt_resizer_hold_repair_tns_pct = ctx.attr.grt_resizer_hold_repair_tns_pct,
        grt_resizer_hold_max_util_pct = ctx.attr.grt_resizer_hold_max_util_pct,
