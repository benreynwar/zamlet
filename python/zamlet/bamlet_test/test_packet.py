...
from zamlet.amlet.packet_utils import packet_to_str, make_coord_register
...
async def packet_send_test(bi: BamletInterface) -> None:
    ...
    await bi.write_register('a', 1, coord_value)  # coordinate/location
    await bi.write_register('a', 2, 3)           # send length
...
async def packet_receive_and_forward_test(bi: BamletInterface) -> None:
    ...
    dest_x, dest_y = 2, 3
    coord_value = make_coord_register(dest_x, dest_y, bi.params.amlet)
    await bi.write_register('a', 4, coord_value)  # new coordinate for forwarding
...
async def packet_receive_forward_and_append_test(bi: BamletInterface) -> None:
    ...
    dest_x, dest_y = 4, 1
    coord_value = make_coord_register(dest_x, dest_y, bi.params.amlet)
    await bi.write_register('a', 3, coord_value)  # new coordinate
    await bi.write_register('a', 2, 3)  # Number to append
    await bi.write_register('d', 6, 0x00ADBEEF)  # data to append (27 bits max)
...
def test_bamlet_packet(verilog_file: str, params_file: str, seed: int = 0) -> None:
    ...
    toplevel = "Bamlet"
    module = "zamlet.bamlet_test.test_packet"
