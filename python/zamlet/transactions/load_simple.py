...
def do_load_simple(jamlet: 'Jamlet', instr: 'Load',
    ...
    ones_word = b'\xff' * word_bytes

    witem_span_id = jamlet.monitor.get_witem_span_id(
        instr.instr_ident, jamlet.k_min_x, jamlet.k_min_y)
    for vline_offset, active_mask, agnostic_mask in vline_offsets_and_masks:
        ...
        # synth_old: 0xFF in agnostic positions, old elsewhere. The
        # subsequent active write then overwrites active positions.
        synth_old = utils.update_bytes_word(
            old_word=old_word, new_word=ones_word, mask=agnostic_mask)
        updated_word = utils.update_bytes_word(
            old_word=synth_old, new_word=new_word, mask=active_mask)
        jamlet.write_vreg(
            rf_word_addr, 0, updated_word,
            span_id=witem_span_id,
            event_details={'mask': f'0x{active_mask:016x}',
                           'agnostic_mask': f'0x{agnostic_mask:016x}',
                           'vline_offset': vline_offset},
        )
        logger.debug(
            ...
            f'instr_ident={instr.instr_ident} active=0x{active_mask:016x} '
            f'agnostic=0x{agnostic_mask:016x}')
