...
class Kamlet:
    ...
    def alloc_dst_pregs(self, base_arch: int, start_vline: int, end_vline: int,
    ...
    async def handle_load_imm_byte_instr(self, instr: kinstructions.LoadImmByte):
        ...
        mask_preg = self.r(instr.mask_reg) if instr.mask_reg is not None else None
        dst_preg = self.rw(instr.dst.reg)
        ...
        reg_offset = dst_preg * wb + instr.dst.offset_in_word
    ...
    async def handle_load_word_instr(self, instr: kinstructions.LoadWord):
        ...
        if is_dst_kamlet:
            # Single-word load with byte_mask — RMW (unmasked bytes preserved
            # from the old value), so dst is allocated via rw().
            mask_preg = self.r(instr.mask_reg) if instr.mask_reg is not None else None
            dst_preg = self.rw(instr.dst.reg)
            read_regs = [mask_preg] if mask_preg is not None else []
            write_regs = [dst_preg]
    ...
    async def handle_load_instr_simple(self, instr: kinstructions.Load) -> None:
        # Compute the vline range and rename arch -> phys for each vline.
        ew = instr.dst_ordering.ew
        elements_in_vline = self.params.vline_bytes * 8 // ew
        start_vline = instr.start_index // elements_in_vline
        end_vline = (instr.start_index + instr.n_elements - 1) // elements_in_vline

        mask_preg = self.r(instr.mask_reg) if instr.mask_reg is not None else None
        dst_pregs = self.alloc_dst_pregs(
            base_arch=instr.dst, start_vline=start_vline, end_vline=end_vline,
            start_index=instr.start_index, n_elements=instr.n_elements,
            elements_in_vline=elements_in_vline,
            mask_present=instr.mask_reg is not None)

        read_regs = [mask_preg] if mask_preg is not None else []
        logger.debug(f'{self.clock.cycle}: kamlet ({self.min_x},{self.min_y}): handle_load_instr_simple '
                       f'instr_ident={instr.instr_ident} WAITING for dst_pregs={dst_pregs} read_regs={read_regs}')
        await self.wait_for_rf_available(write_regs=dst_pregs, read_regs=read_regs,
                                         instr_ident=instr.instr_ident)
        rf_ident = self.rf_info.start(read_regs=read_regs, write_regs=dst_pregs)
        logger.debug(f'{self.clock.cycle}: kamlet ({self.min_x},{self.min_y}): handle_load_instr_simple '
        ...
        if can_read:
            ...
            for jamlet in self.jamlets:
                do_load_simple(jamlet, instr, dst_pregs, mask_preg)
            self.rf_info.finish(rf_ident, write_regs=dst_pregs, read_regs=read_regs)
            self.monitor.finalize_kinstr_exec(instr.instr_ident, self.min_x, self.min_y)
        else:
            witem = WaitingLoadSimple(
                instr=instr, dst_pregs=dst_pregs, mask_preg=mask_preg, rf_ident=rf_ident)
            self.monitor.record_witem_created(
                instr.instr_ident, self.min_x, self.min_y, 'WaitingLoadSimple',
                read_regs=read_regs, write_regs=dst_pregs)
    ...
    async def handle_load_instr_notsimple(self, instr: kinstructions.Load) -> None:
        ...
        mask_preg = self.r(instr.mask_reg) if instr.mask_reg is not None else None
        # Witems indexed by absolute vline (mapping.reg_v); helper returns a
        # list in vline order, convert to dict.
        dst_preg_list = self.alloc_dst_pregs(
            base_arch=instr.dst, start_vline=start_vline, end_vline=end_vline,
            start_index=instr.start_index, n_elements=instr.n_elements,
            elements_in_vline=elements_in_vline,
            mask_present=instr.mask_reg is not None)
    ...
    async def handle_store_instr_simple(self, instr: kinstructions.Store) -> None:
        ew = instr.src_ordering.ew
        elements_in_vline = self.params.vline_bytes * 8 // ew
        start_vline = instr.start_index // elements_in_vline
        end_vline = (instr.start_index + instr.n_elements - 1) // elements_in_vline
        n_vlines = end_vline - start_vline + 1

        src_pregs = [self.r(instr.src + start_vline + i) for i in range(n_vlines)]
        mask_preg = self.r(instr.mask_reg) if instr.mask_reg is not None else None

        read_regs = list(src_pregs)
        if mask_preg is not None:
            read_regs.append(mask_preg)
        await self.wait_for_rf_available(read_regs=read_regs, instr_ident=instr.instr_ident)
        rf_read_ident = self.rf_info.start(read_regs=read_regs)
        if self.cache_table.can_write(instr.k_maddr, writeset_ident=instr.writeset_ident):
            for jamlet in self.jamlets:
                do_store_simple(jamlet, instr, src_pregs, mask_preg)
            ...
            witem = WaitingStoreSimple(
                instr=instr, src_pregs=src_pregs, mask_preg=mask_preg, rf_ident=rf_read_ident)
            self.monitor.record_witem_created(
    ...
    async def handle_store_instr_notsimple(self, instr: kinstructions.Store) -> None:
        ...
        src_pregs = {v: self.r(instr.src + v) for v in range(start_vline, end_vline + 1)}
        mask_preg = self.r(instr.mask_reg) if instr.mask_reg is not None else None
