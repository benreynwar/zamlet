...
def run_test(data_ew: int, index_ew: int, vl: int, n_pages: int, params: ZamletParams, seed: int,
             start_index: int = 0, lmul: int = 1, use_mask: bool = True,
             dump_spans: bool = False, max_cycles: int = 100000):
    """Helper to run a single test configuration."""
    async def test_fn(clock: Clock, lamlet: Oamlet):
        return await run_ordered_indexed_store_test(
...
def test_ordered_indexed_store(params, data_ew, index_ew, vl, n_pages, start_index, lmul, seed):
