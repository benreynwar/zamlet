"""
Test indexed vector load operations with page fault handling.

Tests LoadIndexedUnordered instruction with mixed page types:
...
async def run_indexed_load_test(
    ...
    """Test indexed vector load operations with mixed page types."""
    ...
    await setup_index_register(lamlet, index_reg, indices, index_ew, src_base, span_id)

    # Clear non-idempotent access log before the load
    lamlet.scalar.non_idempotent_access_log.clear()

    result = await lamlet.vload_indexed_unordered(
        vd=data_reg,
        base_addr=src_base,
        index_reg=index_reg,
        index_ew=index_ew,
    ...
    # should have been read (order doesn't matter for unordered loads)
    ...
    if n_expected_correct > 0:
        ...
        for i in range(n_expected_correct):
            ...
            if result != src_list[i]:
                errors.append(f"  [{i}] expected={src_list[i]:#x} actual={result:#x}")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Test indexed vector load with page faults')
