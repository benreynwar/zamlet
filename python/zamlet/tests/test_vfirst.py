"""
Test vfirst.m (find first active set mask bit).

Exercises the ReduceSync (MIN_EL_INDEX) path: each trial plants a random
mask + optional v0, then vfirst.m reports the lowest element index whose
mask bit and (when vm=0) v0 bit are both set, or -1 if none.

A single test case kicks off many trials concurrently: per-trial mask regs
and rd registers are distinct. vm=0 trials share v0 and naturally serialize
on that register.
"""
...
async def _issue_trial_vfirst(lamlet, trial: Trial, page_bytes: int):
    """Issue VfirstM, loading v0 first if vm=0. vm=0 trials serialize on v0."""
    lamlet.vl = trial.vl
    if trial.vm == 0:
        await setup_mask_register(
            lamlet, mask_reg=0, mask_bits=trial.v0_bits[:trial.vl],
            page_bytes=page_bytes, mask_mem_addr=_trial_v0_addr(trial, page_bytes))
    instr = VfirstM(rd=trial.rd, vs2=trial.mask_reg, vm=trial.vm)
    await instr.update_state(lamlet)


def _expected(trial: Trial) -> int:
    for i in range(trial.vl):
        if trial.mask_bits[i] and (trial.vm == 1 or trial.v0_bits[i]):
            return i
    return -1
...
def _build_trials(rnd: Random, params: ZamletParams) -> list:
    ...
    for i in range(TRIALS_PER_TEST):
        ...
        vm = rnd.choice([0, 1])
        # Mix patterns: all-zero, all-one, sparse, dense.
        pattern = rnd.choice(['sparse', 'dense', 'allzero', 'allone', 'random'])
        if pattern == 'allzero':
            mask_bits = [False] * max_bits
        elif pattern == 'allone':
            mask_bits = [True] * max_bits
        elif pattern == 'sparse':
            mask_bits = [rnd.random() < 0.1 for _ in range(max_bits)]
        elif pattern == 'dense':
            mask_bits = [rnd.random() < 0.9 for _ in range(max_bits)]
        else:
        ...
        trial = Trial(
            ...
            expected=0)
        trial.expected = _expected(trial)
        trials.append(trial)
    return trials
...
async def _run_inner(lamlet, seed: int, params: ZamletParams):
    ...
    for trial in trials:
        await _issue_trial_vfirst(lamlet, trial, params.page_bytes)
    ...
    for trial in trials:
        ...
        actual = int.from_bytes(rd_bytes, byteorder='little', signed=True)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Test vfirst.m')
