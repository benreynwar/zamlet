...
async def main(
    ...
    exit_code = await run_strided_test(clock, ew, vl, stride, j_rows, seed)
...
def run_test(ew: int, vl: int, stride: int, j_rows: int = 1, seed: int = 0):
    ...
    exit_code = asyncio.run(main(clock, ew, vl, stride, j_rows, seed))
...
def test_strided(ew, vl, stride):
    run_test(ew, vl, stride)
...
if __name__ == '__main__':
    ...
    try:
        logger.info(
            ...
            f'j_rows={args.j_rows}, seed={args.seed}'
        )
        exit_code = asyncio.run(main(clock, args.ew, args.vl, args.stride, args.j_rows, args.seed))
