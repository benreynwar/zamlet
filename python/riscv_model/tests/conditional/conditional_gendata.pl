...
sub printArray
{
  ...
  my $section = "";
  if ($type eq "int8_t") {
    $section = " __attribute__((section(\".data.vpu8\"), aligned(VLMAX_BYTES)))";
  } elsif ($type eq "int16_t") {
    $section = " __attribute__((section(\".data.vpu16\"), aligned(VLMAX_BYTES)))";
  } elsif ($type eq "int32_t") {
    $section = " __attribute__((section(\".data.vpu32\"), aligned(VLMAX_BYTES)))";
  } elsif ($type eq "int64_t") {
    $section = " __attribute__((section(\".data.vpu64\"), aligned(VLMAX_BYTES)))";
  }

  print $type." ".$arrayName."[DATA_SIZE]".$section." =\n";
  print "{\n";
