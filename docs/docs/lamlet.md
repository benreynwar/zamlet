...
**Cracker** adds kinstructions that free the temp registers once it is done with them, allowing the
slots to be available during renaming.

Once the kinstructions are created they are sent to one or more of the following:
...
The kamlet router connects to the kamlet mesh network.  This is used to broadcast the kinstructions
to the kamlet mesh.
...
reached.  The **Ordered Window** module tracks 16 elements in parallel.  The
...
or a conflicting address.  Element 42 has already received a Page Fault response back
...
or writes to the scalar memory), and the **Ordered Window**.  Loads are directly forwarded to the 
memory system.  For Stores it first waits for the processor store buffer to empty.  Here I'm just
doing what the saturn VPU does.  I don't understand how this guarantees correct order, but assume
that if I'm interfacing with shuttle in the same way then it will be correct.

## Jamlet Router

The jamlet router connects to the jamlet mesh network.  This enables jamlets to communicate with
the **Scalar Memory Interface**, the **Waiting Table** and the **Ordered Window**.

## Synchronizer

A synchronizer connects the lamlet to the [synchronization network](synchronization.md).  This is used by the **Waiting
