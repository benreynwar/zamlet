...
Additionally it can be combined with the systolic arrays of neighboring jamlets to form a larger
toroidal systolic array, and in the limit all the arrays in the Zamlet VPU can act as a single
...
operation was running, and then would be ready to quickly initialize the next operation when it
was finished.  Similarly the systolic arrays would contain two sets of accumulators and one would be
gradually output, while the other was accumulating.

Small matrix-matrix multiplications using a single jamlet's systolic array would be extremely
...
as the operands traverse the much larger systolic array.
